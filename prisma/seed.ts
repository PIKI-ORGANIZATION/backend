...
async function getAdmin() {
  return prisma.akun.findUniqueOrThrow({ where: { email: "admin@dpp.id" } });
}
...
async function seedAllCabang() {
  ...
  for (const c of CABANG_LIST) {
    ...
    // 1f. Akun Admin
    const adminEmail = c.isPusat ? "admin@dpp.id" : `admin_${c.kotaEmail}@dpp.id`;
...
async function assignRolesToAkun() {
  ...
  const adminPusat = await prisma.akun.findUnique({ where: { email: "admin@dpp.id" } });
  const userPusat  = await prisma.akun.findUnique({ where: { email: "user@dpp.id" } });
